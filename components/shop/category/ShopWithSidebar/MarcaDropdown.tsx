...
export const MarcaDropdown = ({ marcas }: Props) => {
    ...
    return (
        ...
        <div className="bg-white shadow-md border border-gray-100 rounded-sm">
            ...
            <div
                className={`${toggleDropdown ? "flex w-full" : "hidden"}`}
            >
                <div className="w-full flex flex-col py-3 pl-6 pr-5">
                    {marcas.map((m) => (
                        <MarcaItem key={m.slug} item={m} />
                    ))}
                </div>
            </div>
        </div>
