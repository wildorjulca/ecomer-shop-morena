import { Categories } from "@/src/interface/searchDidebar";
...
const DropdownItem = ({ category }: CategoryItem) => {
    ...
    const subCategorySlug = searchParams.get("categoria") || "N/A."

    const isSelected = subCategorySlug === category.slug
...
export const CategoryDropdown = ({ categories }: Props) => {
    ...
    return (
        ...
        <div className="bg-white shadow-md border border-gray-100 rounded-sm">
            ...
            >
                <p className="text-gray-900 font-normal">Categorias</p>
                <button
            ...
            >
                {categories.map((item) => (
                    <DropdownItem key={item.slug} category={item} />
                ))}
            </div>
        </div>
