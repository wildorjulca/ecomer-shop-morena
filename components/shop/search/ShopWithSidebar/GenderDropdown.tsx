...
import { Gender } from "@/src/interface/searchDidebar";
...
const DropdownItem = ({ gender }: GenderItem) => {
    ...
    const subCategorySlug = searchParams.get("genero") || "N/A."

    const isSelected = subCategorySlug === gender.slug
    ...
    const handleNavigate = (valor: string) => {
        ...
        console.log(params.get("genero"))

        if (isSelected) {
            params.delete("genero")
            // router.replace()
        } else {
            params.set("genero", valor)
        }
    ...
    return (
        <div
            onClick={() => handleNavigate(gender.slug)}
...
export const GenderDropdown = ({ genders }: Props) => {
    ...
    return (
        ...
        <div className="bg-white shadow-md border border-gray-100 rounded-sm">
            ...
            >
                <p className="text-gray-900 font-normal">Genero</p>
                <button
            ...
            >
                {genders.map((item) => (
                    <DropdownItem key={item.slug} gender={item} />
                ))}
            </div>
        </div>
